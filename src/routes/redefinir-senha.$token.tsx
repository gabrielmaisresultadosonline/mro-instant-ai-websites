...
export const Route = createFileRoute("/redefinir-senha/$token")({
  ssr: false,
  head: () => ({ meta: [{ title: "Redefinir senha — MRO.BIO" }] }),
  component: ResetPage,
});
      ...
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-white/5 p-7">
        ...
        {isLoading ? (
          ...
          <form onSubmit={onSubmit} className="mt-5 space-y-4">
            <p className="text-sm text-white/70">Crie uma nova senha para <strong>{data.email}</strong>.</p>
            ...
            <label className="block">
              <span className="mb-1 block text-xs font-semibold uppercase tracking-wide text-white/60">Confirmar</span>
