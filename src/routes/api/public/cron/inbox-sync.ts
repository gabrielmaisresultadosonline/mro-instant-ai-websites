...
/**
 * Sincroniza a caixa catch-all (*@mro.bio) via IMAP e distribui cada e-mail
 * para o site correspondente (nomedosite@mro.bio -> sites.slug = nomedosite).
 *
...
const MAIL_DOMAIN = (process.env.INBOX_DOMAIN || "mro.bio").toLowerCase();
...
function collectLocalParts(values: (string | null | undefined)[]): string[] {
  const out: string[] = [];
  const re = new RegExp(`([a-z0-9][a-z0-9._-]{0,63})@${MAIL_DOMAIN.replace(/\./g, "\\.")}`, "gi");
...
async function run(): Promise<Response> {
  ...
  if (!user || !pass) {
    return jsonResponse({ ok: false, error: "IMAP_USER/IMAP_PASS não configurados" }, 500);
  }
    ...
    try {
      ...
      for (const message of messages) {
        ...
        const { error: insertError } = await supabaseAdmin.from("site_inbox").insert({
          ...
          to_address: `${site.slug}@${MAIL_DOMAIN}`,
  ...
  return jsonResponse({ ok: true, inserted, skipped, lastUid: maxUid });
}
